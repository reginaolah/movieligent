...
const useSearchResult = (queryString: string, page: number = 1): ReturnType => {
	const [searchResult, setSearchResult] = useState<ISearchResult>({ movies: [], totalPages: 0 });
	const [isLoading, sestIsLoading] = useState<boolean>(true);
	...
	}, [sestIsLoading, setSearchResult, queryString, page]);

	useEffect(() => {
		if (queryString.length > 3) {
			setSearchResult(null);
			sestIsLoading(true);
			debounce(getSearchResult, page === 1 ? 3000 : 1000);
		}
